...
/// <summary>Aggregate Root — Customer.</summary>
public sealed class Customer : Entity<Guid>, IAggregateRoot
{
    ...
    private Customer(Guid id, string firstName, string lastName, string email,
        ...
        Address? shippingAddress, DateTime createAt, DateTime? updateAt) : base(id)
    {
        FirstName = firstName;
        LastName = lastName;
        Email = Email.Create(email);
        PhoneNumber = phoneNumber is not null ? PhoneNumber.Create(phoneNumber) : null;
        ...
        Address? shippingAddress = null)
    {
        ...
        catch (DomainException) { return Result.Failure<Customer>(CustomerErrors.InvalidEmail); }

        PhoneNumber? phoneVo = null;
        if (!string.IsNullOrWhiteSpace(phoneNumber))
        {
            try { phoneVo = PhoneNumber.Create(phoneNumber); }
            catch (DomainException) { return Result.Failure<Customer>(CustomerErrors.InvalidPhoneNumber); }
        }
        ...
        customer.RaiseDomainEvent(new CustomerCreatedEvent(customer.Id, customer.Email.Value, customer.GetFullName()));
        return customer;
    }
    ...
    {
        if (address is null) throw new DomainException("Địa chỉ thanh toán không được null.");
    ...
    {
        if (address is null) throw new DomainException("Địa chỉ giao hàng không được null.");
    ...
    {
        if (newTier <= Tier)
            throw new DomainException(CustomerErrors.CannotDowngradeTier(Tier.ToString(), newTier.ToString()).Description);

        var old = Tier;
        Tier = newTier;
        UpdatedAt = DateTime.UtcNow;
        RaiseDomainEvent(new CustomerTierUpgradedEvent(Id, old, newTier));
    }

    public void AddLoyaltyPoints(int points)
    {
        if (points <= 0) throw new DomainException("Số điểm phải lớn hơn 0.");
    ...
    public void RedeemLoyaltyPoints(int points)
    {
        ...
        if (LoyaltyPoints < points)
            throw new DomainException(CustomerErrors.InsufficientLoyaltyPoints(LoyaltyPoints, points).Description);
    ...
    {
        if (IsActive) throw new DomainException(CustomerErrors.AlreadyActive(Id).Description);
    ...
    {
        if (!IsActive) throw new DomainException(CustomerErrors.AlreadyInactive(Id).Description);
